...
class AppMonitorService : AccessibilityService() {
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent) {
        ...
        // Own app (the SupportBubble Activity) in foreground: hide the bubble (Bug 6).
        // The bubble overlay is FLAG_NOT_FOCUSABLE and therefore never emits
        // WINDOW_STATE_CHANGED itself, so this branch can only be reached by the real
        // SupportBubble UI — never by our own floating bubble.
        if (pkg == applicationContext.packageName) {
            ...
            Log.i(LOG_CURRENT_APP_CHANGED, "currentApp → $pkg (own app)")
            return
        }
